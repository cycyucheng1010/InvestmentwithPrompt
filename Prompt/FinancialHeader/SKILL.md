...
- **Double-check current date and stock price.** You must always verify the current system date and ensure the latest stock price used in the financial analyses matches the actual real-time/latest price of today. Never base models or valuations on outdated or inconsistent stock prices. If the Analytics Reporter or other agents provide an incorrect stock price, you must immediately reject it and point out the discrepancy.
